...
class _VisualProductCustomizeDialogState extends State<VisualProductCustomizeDialog> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (!_availableMilks.contains(_milk)) {
      if (_availableMilks.isNotEmpty) _milk = _availableMilks.first;
      else _milk = 'ไม่ใส่นม';
    }
    ...
    try {
      ...
      if (doc.exists && mounted) {
        ...
        if (data['availableTypes'] != null) {
          ...
          if (realTypes.isNotEmpty) {
            setState(() {
              ...
              if (!_availableTypes.contains(_type)) _autoSelectType();
            });
          }
        }
        ...
        if (data['availableMilks'] != null) {
           List<String> realMilks = List<String>.from(data['availableMilks']);
  ...
  Color _getDrinkColor() {
    Color baseColor;
    if (widget.menu.category == 'ผลไม้') baseColor = Colors.orangeAccent; 
    else if (widget.menu.category == 'ชา') baseColor = const Color(0xFFB77B5E); 
    else if (widget.menu.category == 'นมสด') baseColor = Colors.white; 
    else baseColor = const Color(0xFF3E2723); 

    if (_milk.contains('นม') && widget.menu.category != 'นมสด') {
          ...
          children: [
            ...
            Expanded(
              ...
              child: Stack(
                ...
                children: [
                  Container(decoration: const BoxDecoration(borderRadius: BorderRadius.vertical(top: Radius.circular(25)), gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFFFFF3E0), Colors.white]))),
                  Positioned(top: 20, child: Column(children: [Text(widget.menu.name, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF5D4037))), Text("$_type ฿${finalPrice.toStringAsFixed(0)}", style: const TextStyle(fontSize: 18, color: Colors.brown, fontWeight: FontWeight.bold))])),
                  Positioned(bottom: 20, child: SizedBox(width: 180, height: 250, child: Stack(alignment: Alignment.bottomCenter, children: [AnimatedBuilder(animation: _liquidAnimation, builder: (context, child) { return AnimatedContainer(duration: const Duration(milliseconds: 500), width: 140 + _liquidAnimation.value, height: _type == 'ร้อน' ? 150 : 220, decoration: BoxDecoration(color: _getDrinkColor(), borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(30), bottomRight: Radius.circular(30)))); }), Positioned(bottom: 50, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(_getSugarCount(), (index) => Padding(padding: const EdgeInsets.all(2.0), child: Icon(Icons.crop_square, size: 20, color: Colors.white.withOpacity(0.8)))))), Container(width: 150, height: _type == 'ร้อน' ? 160 : 230, decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade400, width: 3), borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(35), bottomRight: Radius.circular(35)), color: Colors.transparent)), if (_type != 'ร้อน') Positioned(top: -20, right: 40, child: Transform.rotate(angle: 0.2, child: Container(width: 10, height: 100, color: Colors.orange))), if (_type == 'ร้อน') const Positioned(top: 20, child: Icon(Icons.air, color: Colors.white54, size: 40))]))),
                ],
              ),
            ),
